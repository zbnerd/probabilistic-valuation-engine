...
- **장점**: Kafka 경량 버전. DB 커넥션 경합 해결 (메시지 경로 분리)
- **단점**: Redis 메모리 기반 → 대용량 메시지 보관 한계. 클러스터 구성 복잡
- **적합**: 소규모, 메시지 보존 기간 짧아도 되는 경우
- **기각 이유**: 현재 PGMQ에서 오프셋/DLQ/retry 개념을 이미 사용 중인데, Redis Streams는 이를 재구현해야 함

### RabbitMQ

- **장점**: Exchange/Queue 토폴로지로 복잡한 메시지 라우팅 가능
- **단점**: 메시지 재처리/오프셋 관리 없음. 한번 소비하면 메시지 삭제
- **적합**: 작업 큐, 이메일 발송 같은 단순 task queue
- **기각 이유**: 방향이 다름. 현재 프로젝트는 이벤트 스트리밍 + 재처리 필요

### Kafka (선택)

- **장점**: 오프셋 기반 메시지 재처리 가능. 파티션 수 = 병렬 처리 단위, 선형 스케일아웃
- **단점**: 운영 복잝도 높음. 소규모에는 과한 설정
- **선택 이유**: PGMQ에서 이미 오프셋/DLQ/retry 패턴 사용 중 → DB 밖으로 꺼내기만 하면 됨. 개념 매핑이 자연스러움

| 항목 | Redis Streams | RabbitMQ | Kafka |
|------|--------------|----------|-------|
| DB 커넥션 분리 | O | O | O |
| 오프셋/재처리 | 제한적 | X | O |
| 선형 스케일아웃 | X (메모리 한계) | X | O (파티션) |
| 운영 복잡도 | 중 | 낮 | 높 |
| 현재 패턴 호환 | 부분 | X | O |
