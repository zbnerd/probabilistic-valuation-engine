...
로컬 개발 환경에서 응답이 2초씩 걸렸다. 동시 사용자가 10명만 넘어도 타임아웃이 났다. Locust로 측정해보니 **약 90~120 RPS**. 이 정도면 트래픽이 조금만 몰려도 서버가 다운된다.
...
4. **스레드풀 부족**: `expectationComputeExecutor`가 core 4, max 8. 100명 동시 요청에 208개 처리 한계
