...
```
# application-prod.yml (당시)
spring:
  datasource:
    hikari:
      maximum-pool-size: 25        ← Production 설정
      minimum-idle: 10
...
server:
  tomcat:
    threads:
      max: 200                     ← HikariCP의 8배
```

**[실측] HikariCP 풀 사이즈(25)가 Tomcat 스레드 풀(200)에 비해 작았다.**
...
(이 타임라인은 설명을 위한 예시입니다. 실제 측정값이 아닙니다)
```

요청 처리 시간의 **[예시] 97%** 가 커넥션 대기 시간이었다.
...
200개 스레드가 동시에 캐시 미스를 겪으면, 25개 커넥션으로는 절대 부족하다.

## 모니터링에서 포착된 패턴

```
[예시] HikariCP Pool Usage Over Time:
