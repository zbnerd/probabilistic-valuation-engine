...
## Project 1: 좋아요 토글(도메인) 고부하 지연+서버 종료 시 데이터 유실(문제)을 낙관적 락 전환+Graceful Shutdown(해결)으로 안정화+Scale-out 기반 마련(결과)
...
**[4장] 구현:**
Proxy/Decorator 패턴으로 캐싱·동시성·로깅 계층을 분리. Spring @PreDestroy 훅으로 SIGTERM 수신 시 버퍼를 먼저 flush하고 안전 종료. pg_try_advisory_xact_lock 기반 분산 락 AOP로 Scale-out 시 스케줄러 중복 실행 방지. TieredCache(L1 Caffeine → L2 Redis → SingleFlight → Loader) 구조 도입.

**[5장] 결과:**
비관적 락 지연 해결, 서버 종료 시 데이터 유실 방지, 관측 가능성 확보(Micrometer 4개 핵심 메트릭). Phase 1 통합 릴리즈 완료. 단, Check-Then-Act TOCTOU 레이스 컨디션, Relation과 Counter의 비원자적 이중 쓰기, unlike 시 동기 DB DELETE 문제가 여전히 남아 있었다.
...
## Project 2: 좋아요 수 중복 카운트+fetchAndClear 비원자성(문제)을 Redis Lua Script 원자화+보상 트랜잭션(해결)으로 데이터 정합성 100% 확보(결과)
...
```
비원자적 fetchAndClear의 위험:
Step 1: Buffer에서 데이터 읽기 (fetch)
Step 2: Buffer에서 데이터 삭제 (clear)
Step 3: DB에 데이터 쓰기 (persist)
← Step 2와 3 사이에 크래시 → 중복 카운트 or 데이터 유실
```
...
**[5장] 결과:**
fetchAndClear 간극 0달성(비원자적→Lua 원자 실행). Sync 소요시간 200~500ms→50~150ms. Redis 명령 수 15~20→5~8. DB 쿼리 N+1→Batch. 순환 참조 해결. 보상 트랜잭션으로 DB 실패 시에도 데이터 보호. BYOK 인증으로 self-like 방지의 전제 조건도 확보.
...
## Project 3: 좋아요 토글(도메인) TOCTOU 레이스컨디션+Scale-out 불가(문제)를 Atomic Lua Script+Redis Pub/Sub(해결)로 DB QPS 12-17x 감소, P99 35ms→8ms 개선(결과)
...
│ HikariCP 사용률  │ 75-125%     │ 10-15%   │ 7x ↓    │
```

---

## Project 4: 비즈니스 로직과 인프라 혼재(문제)를 멀티모듈 분리+Kotlin 마이그레이션(해결)로 도메인 격리, 순수 단위 테스트 가능(결과)
...
## Project 5: 모듈 간 강결합(문제)을 헥사고날 아키텍처 Port/Adapter(해결)로 인프라 독립 달성, Redis→PostgreSQL 전환 시 module-core 0줄 변경(결과)
...
## Project 6: Redis 운영 복잡도+장애 이중화(문제)를 PostgreSQL PGMQ+UNLOGGED TABLE 전환(해결)으로 Redis 의존성 완전 제거, module-core 0줄 변경(결과)
...
## Project 7: Buffer+PGMQ 경유 복잡도+like_count 불일치(문제)를 Direct DB Transaction+Trigger(해결)로 단일 트랜잭션 원자성 달성, 123일 여정 완결(결과)

**[1장] 문제:**
Redis 제거 후에도 Like 토글은 In-Memory Buffer→PGMQ Publish→Consumer→DB Write의 복잡한 경로를 거쳤다. 로컬 상태(Buffer)와 메시지 유실 가능(PGMQ)이 여전히 존재했다. INSERT character_like와 UPDATE game_character.like_count가 별도 SQL이라 둘 사이에 실패하면 count가 틀어졌다. Self-like 방지도 현재 로그인 캐릭터 1개만 차단해서, 같은 계정의 다른 캐릭터로 우회 가능했다.
...
2026.03: Controller → DB Transaction + Trigger → 완료
