...
- **Cleanup durability gap:** `ConsumedChunkInbox` is in-memory, acknowledges Kafka after enqueue, and can drop the oldest entry at capacity. It conflicts with the repository's current MQ durability rule; do not use it as evidence of end-to-end exactly-once cleanup.
