...
- Enumeration: authenticated GitHub REST Search `GET /search/issues?q=repo:zbnerd/probabilistic-valuation-engine+is:issue&per_page=100` with `--paginate`; independently checked against `total_count`.
- Counts: Search enumeration 752 unique issue numbers; Search `total_count` 752; duplicate count 0. GraphQL repository `issues.totalCount` 752.
...
- Textual references to PR/commit numbers are not promoted to formal links; formal linkage is only GitHub’s closed-by-PR relation.
