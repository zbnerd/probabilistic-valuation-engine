...
`SnapshotChunkProcessor` 의 `parseLines`(JSON parse) 와 `processItems`(확률 계산) worker count 를 독립 설정 가능하게 분리한다. 기존 `workerCount` 기본값은 그대로 유지하여 backward-compat 보장.
...
| `parseDispatcher` | CoroutineDispatcher | `Dispatchers.Default` | ADR-723 §23.2 Algorithm-based 분류: O(n) JSON parse, n=10K+ 무조건 offload |
| `calcDispatcher` | CoroutineDispatcher | `Dispatchers.Default` | ADR-723 §23.2: 확률 계산은 무조건 offload (모든 크기) |
...
- ❌ `workerCount` legacy field 제거 (별도 deprecation issue, 후속)
- ❌ channel capacity 분리 (per-stage `channel-capacity-parse`, `channel-capacity-calc`, 별도 issue)
- ❌ parseDispatcher/calcDispatcher 별도 bean 등록 (현재 inline default, YAML override 가능)
...
- [x] Internal consistency: 4 결정 (default, dispatcher, legacy) 정합
- [x] Scope: 단일 PR, bounded
- [x] Ambiguity: dispatcher override type 명확 (CoroutineDispatcher)
