 ...
 * <h3>왜 별도 Bean인가?</h3>
 *
 * <p>Spring AOP는 프록시 기반으로 동작하므로, 같은 클래스 내에서 {@code this.method()} 호출(self-invocation)하면 AOP가 적용되지
 * 않습니다. 각 스테이지를 별도 Spring Bean의 <b>public 메서드</b>로 분리하면 프록시를 정상적으로 통과하여 {@code @TimedStage}가 올바르게
 * 동작합니다.
 *
...
public class CalculationStageDelegate {
  ...
  public void persist(String userIgn, Object result) {
    ...
    executor.executeVoidJava(() -> log.debug("[Stage:persist] userIgn={}", userIgn), context);
  }
  ...
  public void upsertView(String userIgn, Object result) {
    ...
    executor.executeVoidJava(() -> log.debug("[Stage:view_upsert] userIgn={}", userIgn), context);
  }
  ...
  public void publishEvent(String userIgn, Object result) {
    ...
    executor.executeVoidJava(() -> log.debug("[Stage:event_publish] userIgn={}", userIgn), context);
  }
}
