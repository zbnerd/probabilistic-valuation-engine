...
class TailProbabilityCalculator {
     ...
     * @return P(X >= target)
     */
    fun calculateTailProbability(pmf: DensePmf, target: Int, tailClampApplied: Boolean): Double = if (tailClampApplied) {
        pmf.massAt(target)
    } else {
     ...
     */
    fun calculateExpectedTrials(probability: Double): Double = if (probability <= 0.0) {
     ...
     */
    fun calculateExpectedTrialsCeil(probability: Double): Long = if (probability <= 0.0) {
        Long.MAX_VALUE
    } else {
