...
class CharacterBasicFetchPhaseTest {
    ...
    fun `execute writes exact endpoint marker before creating sink`() {
        ...
        whenever(objectStorage.listByPrefix("character-basic/")).thenReturn(emptyList())
        ...
        try {
            awaitFuture(phase.execute(executor, mapOf("ign" to "ocid"), RUN_ID))
        } finally {
            executor.shutdownNow()
        }

        verify(objectStorage).put(eq("runs/run-character/character-basic/_RUNNING"), any<ByteArray>())
        verify(sinkFactory).createForCharacterBasic(RUN_ID)
    }

    @Test
    fun `marker failure prevents character basic sink creation`() {
        val objectStorage = mock<ObjectStorage>()
        whenever(objectStorage.listByPrefix("character-basic/")).thenReturn(emptyList())
        ...
        verify(sinkFactory, never()).createForCharacterBasic(any())
    }

    private fun phase(objectStorage: ObjectStorage, sinkFactory: EndpointSinkFactory): CharacterBasicFetchPhase =
        CharacterBasicFetchPhase(
    ...
    private companion object {
        const val RUN_ID: String = "run-character"
    }
}
