...
class AlertThrottler(
    ...
    private fun buildDailyCountKey(): String = "{ai-throttle}:daily-count:" + LocalDate.now()

    private fun buildPatternTimesKey(): String = "{ai-throttle}:pattern-times"
    ...
    fun canSendAiAnalysis(): Boolean = executor.executeOrDefault(
    ...
    fun shouldSendAlert(errorPattern: String): Boolean = executor.executeOrDefault(
    ...
    fun canSendAiAnalysisWithThrottle(errorPattern: String): Boolean = shouldSendAlert(errorPattern) && canSendAiAnalysis()

    fun getDailyUsage(): Int = executor.executeOrDefault(
        { fetchDailyUsage() },
        0,
        TaskContext.of("AlertThrottler", "GetUsage"),
    )

    private fun fetchDailyUsage(): Int = getDailyCounter().get().toInt()

    fun getRemainingCalls(): Int = maxOf(0, dailyLimit - getDailyUsage())
