...
open class CharacterValuationEntity {
    ...
    companion object {
        /**
         * Create entity from MongoDB CharacterValuationView
         *
         * <p>Used during migration from MongoDB to PostgreSQL.
         *
         * @param view MongoDB view to migrate
         * @return new JPA entity with data from view
         */
        fun fromMongoView(view: maple.expectation.infrastructure.mongodb.CharacterValuationView): CharacterValuationEntity {
            val entity = CharacterValuationEntity(
                userIgn = view.userIgn,
                calculatedAt = view.calculatedAt,
                version = view.version ?: 0L,
            )
            entity.messageId = view.messageId
            entity.characterOcid = view.characterOcid
            entity.characterClass = view.characterClass
            entity.characterLevel = view.characterLevel
            entity.lastApiSyncAt = view.lastApiSyncAt
            entity.lastAppliedVersion = view.lastAppliedVersion
            entity.totalExpectedCost = view.totalExpectedCost
            entity.maxPresetNo = view.maxPresetNo
            entity.fromCache = view.fromCache
            entity.presets = view.presets?.let { serializePresets(it) }
            return entity
        }

        /**
         * Serialize presets list to JSON string.
         *
         * <p>Simple JSON serialization without external dependencies.
         * For complex nested structures, consider using Jackson or Gson.
         */
        private fun serializePresets(
            presets: List<maple.expectation.infrastructure.mongodb.CharacterValuationView.PresetView>,
        ): String {
            // Simple manual JSON serialization to avoid Jackson Kotlin module dependency
            val sb = StringBuilder("[")
            presets.forEachIndexed { index, preset ->
                if (index > 0) sb.append(",")
                sb.append("{")
                sb.append("\"presetNo\":").append(preset.presetNo).append(",")
                sb.append("\"totalExpectedCost\":").append(preset.totalExpectedCost).append(",")
                sb.append("\"totalCostText\":\"").append(preset.totalCostText ?: "").append("\",")
                sb.append("\"costBreakdown\":{")
                sb.append("\"blackCubeCost\":").append(preset.costBreakdown?.blackCubeCost).append(",")
                sb.append("\"redCubeCost\":").append(preset.costBreakdown?.redCubeCost).append(",")
                sb.append("\"additionalCubeCost\":").append(preset.costBreakdown?.additionalCubeCost).append(",")
                sb.append("\"starforceCost\":").append(preset.costBreakdown?.starforceCost).append(",")
                sb.append("\"flameCost\":").append(preset.costBreakdown?.flameCost)
                sb.append("},")
                sb.append("\"items\":[")
                preset.items?.forEachIndexed { itemIndex, item ->
                    if (itemIndex > 0) sb.append(",")
                    sb.append("{")
                    sb.append("\"itemName\":\"").append(item.itemName ?: "").append("\",")
                    sb.append("\"expectedCost\":").append(item.expectedCost).append(",")
                    sb.append("\"costText\":\"").append(item.costText ?: "").append("\"")
                    sb.append("}")
                }
                sb.append("]")
                sb.append("}")
